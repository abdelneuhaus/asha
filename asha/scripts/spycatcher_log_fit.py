...
def plot_log_fits(conc_densities, densities, conc_clusters, clusters, fit_mode="full"):
    """
    Generates a dual-axis plot with linear regression fits on a logarithmic X-axis.
    ...
    Args:
        ...
        fit_mode (str, optional): The fitting range mode. Defaults to "full".

    Returns:
        None: The function generates and displays the matplotlib figure directly.
    """
    ...
    x_den_positive = x_den[mask_den][x_den[mask_den] > 0]
    x_smooth_den = np.logspace(np.log10(x_den_positive.min()), np.log10(x_den_positive.max()), 100)
    fit_den_line = a_den * x_smooth_den + b_den

    ax1.scatter(x_den, y_den, color='steelblue', label='Densité data')
    ax1.plot(x_smooth_den, fit_den_line, color='steelblue', linestyle='--', 
             label=f'Fit Densité {fit_label_suffix} : y={a_den:.2f}x + {b_den:.2f} ($R^2$={r2_den:.3f})')
    
    ax1.set_xscale('log') # Passage en échelle logarithmique
    ax1.set_xlabel('SpyCatcher Concentration (log scale)', fontsize=12, fontweight='bold')
    ...
    x_clu_positive = x_clu[mask_clu][x_clu[mask_clu] > 0]
    x_smooth_clu = np.logspace(np.log10(x_clu_positive.min()), np.log10(x_clu_positive.max()), 100)
    fit_clu_line = a_clu * x_smooth_clu + b_clu

    ax2.scatter(x_clu, y_clu, color='salmon', label='Clusters data')
    ax2.plot(x_smooth_clu, fit_clu_line, color='salmon', linestyle='--', 
...
def process_single_well(args):
    ...
    Args:
        args (tuple): A packed tuple containing the following elements in order:
            - position (str): The alphanumeric well identifier (e.g., "C9").
            - conc (float): The experimental SpyCatcher concentration for the well.
            - list_pt (list[str]): Complete list of absolute file paths to PALMTracer files.
            - list_poca (list[str]): Complete list of absolute file paths to PoCA files.
            - surface (float): The calculated field-of-view surface area in µm².

    Returns:
        tuple[float, float | None, float | None]: A tuple containing:
            - conc (float): The concentration value passed in the arguments.
            - avg_density (float or None): The calculated mean molecular density across replicates, or None if no valid PALMTracer data exists.
            - avg_clusters (float or None): The calculated mean cluster count across replicates, or None if no valid PoCA data exists.
    """
...
if __name__ == '__main__':
    ...
    print("Generating logarithmic fit plot...")
    plot_log_fits(plot_conc_densities, plot_densities, plot_conc_clusters, plot_clusters, fit_mode="0-2")
        ...
        for dir in dirs:
            if dir.startswith('P 0'):
                ...
                for f in range(len(poca_files)):
                    ...
                    raw_file_poca["photon_loc"] = (np.array(raw_file_poca['intensity'] / raw_file_poca['total ON']))*(3.6/300)
                    medians.append(np.median(raw_file_poca["photon_loc"]))#*(3.6/300))
...
def plot_photophysical_medians(repertories, group_name, parameter):
    median_values = {}
    for repertory in repertories:
        full_path = os.path.join('D:/2504_comparison/NO FILTERING', repertory)
        for root, dirs, files in os.walk(full_path):
            for dir in dirs:
                if group_name in dir:  # Vérifie si le groupe correspond
                    poca_files = get_poca_files(os.path.join(root, dir))
                    medians = []
                    for f in range(len(poca_files)):
                        raw_file_poca = read_poca_files(poca_files[f])
                    ...
                    if repertory not in median_values:
                        median_values[repertory] = []
                    median_values[repertory].extend(medians)
...
# repertory = 'D:/2504_comparison/NO FILTERING/PCSTAR'
# median_values = calculate_median_values(repertory)
# plot_median_values(median_values)

repertories = ['PCSTAR', 'MEOS32', 'MEOS4B', 'MEOS4BL93M']
group_name = '10mW'
parameter = 'intensity'
plot_photophysical_medians(repertories, group_name, parameter)